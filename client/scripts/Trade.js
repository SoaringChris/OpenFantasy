tradeObj = function()
{
    ...
    self.display = function(){
        ...
        {
          $("#teamSelect1").append("<option data-teamNo = \""+ i +"\" id='teamSelect1-"+ counter + "' value = '"+ counter +"'>"+global.teams[i].name +"</option>");
          $("#teamSelect2").append("<option data-teamNo = \""+ i +"\" id = 'teamSelect2-"+ counter + "' value = '"+ counter +"'>"+global.teams[i].name +"</option>");
          counter++;
        }
        if(self.team1 != null)
        {
        ...
        if(self.team1 != null)
        {
            ...
            self.team1.players().forEach(function (player, i) {
                if(!self.players1.includes(player)) {
                    plist1.append("<button data-playNo = '" + global.players.indexOf(self.team1.players()[i]) + "' class = \"list-group-item list-group-item-action noCorner mx-0\" onclick='offerPlayer(event, 1)'><div style = \"clear: both\"><p class = 'my-0 alignleft'>" + self.team1.players()[i].name + "</p><p class = \"my-0 alignright\"><i class=\"mt-1-5 fas fa-angle-right\"></i></p></div></button>\n");
                }
            });
        }
        else{ppoint1.html('0')}
    ...
    self.complete = function() {
        self.players1.forEach(function (player, i) {
            {
                player.owner = self.team2;
            }
        });

        self.players2.forEach(function (player, i) {
            {
